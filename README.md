...
# How It Works
PacketWhisper combines DNS queries with text-based steganography to make it all happen. Leveraging the <a href="https://github.com/TryCatchHCF/Cloakify">Cloakify Toolset</a> (text-based steganography), it transforms the payload into a list of FQDN strings. It then transfers data across (or within) network boundaries via those DNS queries, with the data hidden in plain sight, without the two systems ever directly connecting to a each other or to a common endpoint. The ciphers used by PacketWhisper provide multiple levels of deception to avoid generating alerts as well as to mislead analysis attempts.
